...
public class ServerListener implements Runnable{
    ...
    private void login(String username) throws UserLoginException {
    	...
    	try{
	    	if(isLoggedIn(username)){
	    		...
	    		throw new UserLoginException(String.format(ErrorMessages.MAX_USER_LIMIT_REACHED, Constants.MAX_USERS_SUPPORTED));
	    	} else {
    ...
    private void sendLoginSuccessfulResponse() {
    	try{
    		this.objectOutputStream.writeObject(new Message.Builder()
    													   .withText(ResponseMessages.LOGIN_SUCCESSFUL)
										    			   .withMessageType(MessageType.LOGIN_SUCCESSFUL)
										    			   .build());
    	} catch (IOException e) {
    		logger.error(e.getMessage(), e);
    	}
    ...
    private void sendLoginFailedResponse(Throwable cause) {
    	try{
    		this.objectOutputStream.writeObject(new Message.Builder()
    													   .withText(String.format(ResponseMessages.LOGIN_FAILED, cause.getMessage()))
										    			   .withMessageType(MessageType.LOGIN_FAILED)
										    			   .build());
    	} catch (IOException e) {
    		logger.error(e.getMessage(), e);
    	}
    ...
    private void sendTextSentSuccessfulResponse(String receiverId) {
		try{
    		this.objectOutputStream.writeObject(new Message.Builder()
                    									   .withText(String.format(ResponseMessages.TEXT_SENT_SUCCESSFUL, receiverId))    				
										    			   .withMessageType(MessageType.TEXT_SENT_SUCCESSFUL)
										    			   .build());
    	} catch (IOException e) {
    		logger.error(e.getMessage(), e);
    	}
	...
	private void sendTextSentFailedResponse(String receiverId, Throwable cause) {
		try{
    		this.objectOutputStream.writeObject(new Message.Builder()
										    			   ...
										    			   .withMessageType(MessageType.TEXT_SENT_FAILED)
										    			   .build());
    	} catch (IOException e) {
    		logger.error(e.getMessage(), e);
    	}
